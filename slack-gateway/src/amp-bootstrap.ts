...
/**
 * Discover provider info from the maestro URL.
 * Tries /.well-known/agent-messaging.json, then /api/v1/info, then /api/v1/health.
 */
  ...
  for (const ep of endpoints) {
    try {
      ...
      if (resp.ok) {
        ...
        if (result.tenant && result.domain) {
          return result;
        }
      }
    } catch {
...
/**
 * Generate an Ed25519 key pair using Node.js crypto.
 */
...
/**
 * Register with the AMP provider.
 */
...
/**
 * Save agent config and keys to ~/.agent-messaging/agents/{name}/
 */
function saveAgentFiles(
  ...
  const msgsDir = path.join(agentDir, 'messages');
  const inboxDir = path.join(msgsDir, 'inbox');
  const sentDir = path.join(msgsDir, 'sent');
...
/**
 * Update .index.json to map agent name → directory name.
 */
...
/**
 * Persist AMP config to the .env file.
 */
...
/**
 * Auto-bootstrap: discover, generate keys, register, save, persist.
 */
export async function bootstrapAMP(options: BootstrapOptions): Promise<BootstrapResult> {
  ...
  console.log(`[BOOTSTRAP] Maestro: ${options.maestroUrl}`);
  ...
  const result = await registerAgent(
    ...
    options.agentName,
    publicKeyPem,
    options.alias || options.agentName,
    options.metadata || {}
  );
